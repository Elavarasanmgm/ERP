...
const Timesheets = () => {
  ...
  const [formData, setFormData] = useState({
    projectId: '', taskDate: new Date().toISOString().split('T')[0], hoursWorked: '', description: '',
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setFormData({ projectId: '', taskDate: new Date().toISOString().split('T')[0], hoursWorked: '', description: '' });
      setShowForm(false);
      fetchData();
    } catch (err) {
    ...
    <div className="accounting-section">
      ...
      {showForm && (
        <form className="form-card" onSubmit={handleSubmit}>
          ...
          <input type="date" name="taskDate" value={formData.taskDate} onChange={handleInputChange} required />
          <input type="number" name="hoursWorked" placeholder="Hours Worked *" value={formData.hoursWorked} onChange={handleInputChange} step="0.5" required />
          ...
          <tbody>
            {timesheets.map((sheet) => (
                <tr key={sheet.TimesheetId}>
                  ...
                  <td>{new Date(sheet.TaskDate).toLocaleDateString('en-IN')}</td>
